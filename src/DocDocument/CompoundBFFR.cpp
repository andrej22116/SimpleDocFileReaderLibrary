#include <SDRL/DocDocument/CompoundBFFR.hpp>//"windowscompoundbinaryfileformatreader.h"

#include <SDRL/DocDocument/variablevisualize.hpp>//"variablevisualize.hpp"
#include <SDRL/DocDocument/Compound_wcbff_structures.h>//"wcbff_structures.h"
#include <SDRL/DocDocument/inputbinarystream.h>//"ClassInputBinaryStream/inputbinarystream.h"
...
namespace sdrl {
	...
	struct WindowsCompoundBinaryFileFormatReader::_WCBFF_Structure {
		...
		InputBinaryStream<char>& iStream;

		_WCBFF_Structure(InputBinaryStream<char>& _iStream) : iStream(_iStream) {}
	};


	WindowsCompoundBinaryFileFormatReader::WindowsCompoundBinaryFileFormatReader(std::istream& stream)
	{
		InputBinaryStream<char> binStream(stream);
		_WCBFF_Structure fileStructure(binStream);
		read(fileStructure);
	}


	WindowsCompoundBinaryFileFormatReader::WindowsCompoundBinaryFileFormatReader(const std::wstring& filename)
	{
		...
		InputBinaryStream<char> binStream(file);
		_WCBFF_Structure fileStructure(binStream);
		read(fileStructure);
	}
	...
	void WindowsCompoundBinaryFileFormatReader::readDirectoryEntries(_WCBFF_Structure& fileStructure)
	{
		...
		while (sector != WCBFF_EndOfChain)
		{
				...
				offsetInSector += sizeof(WCBFF_DirectoryEntry))
			{
				...
				if ((static_cast<uint64_t>(fileStructure.iStream.base().gcount()) < sizeof(WCBFF_DirectoryEntry))
		...
		readDataToStream(fileStructure, fileStructure.iStream.base(), _filesStreams[0],
	...
	void WindowsCompoundBinaryFileFormatReader::readFilesStreams(_WCBFF_Structure& fileStructure)
	{
		for (size_t streamID = 1; streamID < fileStructure.directoryEntrys.size(); streamID++)
		{
			...
			if (fileStructure.directoryEntrys[streamID].streamSize > 0)
			{
				...
				{
					readDataToStream(fileStructure, fileStructure.iStream.base(), _filesStreams[streamID],
						fileStructure.directoryEntrys[streamID].sectorStartStream,
